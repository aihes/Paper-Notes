# CAMEL Hybrid Browser Toolkit 技术实现分析

本文档旨在分析 CAMEL 项目中 `hybrid_browser_toolkit` 的技术实现，内容覆盖其架构设计、核心功能与通信协议。

## 1. 宏观架构：基于进程隔离的混合式设计

该工具包采用 Python 与 TypeScript (Node.js) 结合的混合架构，通过进程解耦上层逻辑与底层浏览器控制，以此提升系统的稳定性、隔离性与可扩展性。
...
1.  **稳定性 (进程隔离)**: 每个 `HybridBrowserToolkit` 实例会启动一个独立的 Node.js 子进程和浏览器实例。这种设计使得不同浏览器会话在操作系统层面完全隔离，单个会话的崩溃不会影响 Python 主进程或其他会话，从而提升了整体稳定性。

2.  **扩展性 (命令模式)**: 两端通过命令模式进行交互。Python 端将操作（如 `click`）封装成标准 JSON 命令，通过 WebSocket 发送给 Node.js 端执行。这种接口边界定义了清晰的扩展方式：只需在 TypeScript 端添加新命令的实现，并在两端注册该命令，即可增加新功能，无需改动核心通信逻辑。
...
## 2. 核心功能：生成 AI 友好的页面快照

此功能通过一个三步处理流程，将网页转换为 AI 易于处理的结构化数据。其核心是结合了**结构**与**视觉**的双重过滤机制。
...
-   **技术**: 调用 Playwright 的内部函数 `page._snapshotForAI()`。
-   **原理**: 该函数直接遍历浏览器的**无障碍树 (Accessibility Tree)**，而非原始的 HTML DOM。无障碍树为屏幕阅读器等辅助技术而生，本身已过滤了大量纯样式、无语义的元素。
-   **优势**: 从源头上减少了无关信息，获取的数据直接包含元素的角色（role）和名称（name）。
-   **输出**: Playwright 为树中每个节点分配一个唯一的 `[ref=N]` ID，并以保留层级关系的**缩进文本格式**输出。

#### 第二步：解析 - 从文本到图结构

-   **执行者**: `snapshot-parser.ts` 中的 `parseSnapshotHierarchy` 函数。
-   **原理**: 该函数读取缩进文本，通过分析每行的**缩进量**并借助一个**栈（Stack）**来追踪父节点，从而重建完整的父子层级关系。
-   **输出**: 生成一个 `Map<string, SnapshotNode>` 结构的**层级关系图 (Graph)**。该图结构是后续过滤操作的基础，使程序能查询任意元素的父、子节点及其属性。

#### 第三步：过滤 - 结合结构与视觉的双重过滤

此步骤通过两轮过滤来精简快照内容，以确保其准确性。
...
通过这种**结构**与**视觉**相结合的双重过滤，最终呈现给 AI 的可交互元素列表得到简化，同时保证了操作的准确性。
...
## 3. 通信协议：基于 WebSocket 的异步请求/响应

两端之间通过 WebSocket 进行异步通信。Python 端的 `ws_wrapper.py` 为每个发出的命令分配一个唯一的 `id` 和一个 `asyncio.Future` 对象。该机制确保在异步环境中，每个响应都能准确地匹配到其原始请求。

## 总结

`camel.toolkits.hybrid_browser_toolkit` 的核心设计特点可以概括为：
-   **混合架构**：通过 Python 和 Node.js 的进程隔离来保证稳定性。
-   **命令模式与 WebSocket**：实现了标准化的跨语言通信。
-   **AI 友好的快照**：通过一个多阶段处理流程，结合 **DOM 层级**和**几何位置**的**双重过滤机制**，将网页转换为精简的结构化数据。
-   **Set of Marks (SoM)**：为多模态 Agent 提供了从视觉定位到精确操作的连接。

该文档记录了此工具包的关键技术实现，可作为技术参考。
