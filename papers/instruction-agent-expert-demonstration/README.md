...
### 1. 难以理解的非标 UI (Novel/Unintuitive UI)
应用软件中常出现设计不规范或极其生僻的 UI 元素（例如下图），人类用户往往需要通过尝试或查阅文档才能理解其含义。
对于依赖通用训练数据的 Agent 而言，直接推断这些元素的交互逻辑极难。
![Non-intuitive UI](images/figure-2-non-intuitive-ui.png)
*Figure 2: 典型的非直观 UI 示例。左侧图标虽形似播放键，但在此上下文中可能具有完全不同的含义。*

**Instruction Agent 的解决方法：基于坐标的视觉描述生成**
Agent 并不需要真正“理解”这个奇怪图标的含义，它只需要知道“长什么样的东西”是需要被点击的。
1.  **捕获坐标**：记录人类专家点击的确切位置 $(x, y)$。
2.  **视觉标注**：在输入给 Instructor 模型（GPT-4o）的截图中，在 $(x, y)$ 处直接画上标记。
3.  **生成描述**：Prompt 引导模型描述标记处的视觉特征（如“左上角的蓝色圆形图标”）。
4.  **执行定位**：Actor 阶段的 UI-Tars 模型根据这个**视觉描述**进行定位，而非依赖深层的语义理解。

### 2. 长序列操作的累积误差
随着步骤数增加，单步执行的累积误差导致整体成功率指数级下降。

### 3. 个性化配置
通用 Agent 倾向于使用“标准路径”，而实际工作流常涉及特定的用户习惯（如特定浏览器配置、插件使用）。
