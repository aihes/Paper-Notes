...
![Figure 1: Performance Comparison](images/f1.png)
...
![Figure 3: GUI Grounding Pipeline](images/f3.png)
...
![Figure 4: Self-evolving Data Pipeline](images/f4.png)
...
![Figure 5: Agentic RL Framework](images/f5.png)
...
![Figure 6: RL Rollout Process](images/f6.png)

> **图解 Figure 6**：
> 展示了具体的 Rollout 细节：模型（Actor）接收环境的状态（Screenshot + XML），输出动作（Action），环境执行后返回奖励（Reward）。这一循环不断产生数据用于 GRPO 等算法的策略更新。

### 4.2 端云协同架构 

这是 MAI-UI 最具落地价值的设计。纯端侧模型太弱，纯云侧模型太贵且不安全。MAI-UI 选择了“既要又要”。

![Figure 7: Device-Cloud Collaboration](images/f7.png)
...
*   **GUI Grounding**: GUI 定位，指智能体理解屏幕内容并将自然语言指令映射到具体 UI 元素坐标的能力。
*   **MCP (Model Context Protocol)**: 一种开放标准，允许 AI 模型以标准化的方式连接和使用外部数据源或工具（如日历、地图、GitHub 等）。
*   **SFT (Supervised Fine-Tuning)**: 有监督微调，使用标注好的数据对预训练模型进行针对性训练。
*   **RL (Reinforcement Learning)**: 强化学习，通过让智能体与环境交互并根据奖励信号来优化策略。
*   **GRPO (Group Relative Policy Optimization)**: 一种强化学习算法，通过比较一组采样结果的相对优势来更新策略，常用于大模型推理能力的训练。
*   **AVD (Android Virtual Device)**: Android 虚拟设备，用于模拟 Android 手机运行环境。
*   **CoT (Chain-of-Thought)**: 思维链，一种提示工程技术，引导模型逐步推理出答案。

## 参考文献 (References)

*   Zhou, H., Zhang, X., Tong, P., Zhang, J., Chen, L., Kong, Q., ... & Hoi, S. (2025). MAI-UI Technical Report: Real-World Centric Foundation GUI Agents. arXiv preprint arXiv:2512.22047.
